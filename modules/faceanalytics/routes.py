...
from modules.faceanalytics.service import FaceAnalyticsService
from modules.faceanalytics.schema import (
    FaceAnalyticsSessionResponse,
...
router = APIRouter(prefix="/faceanalytics", tags=["Face Analytics & Attendance"])
...
async def upload_batch_cctv_footage(
    ...
    service = FaceAnalyticsService(db)
...
@router.post(
    "/process",
    response_model=StandardResponse[List[FaceAnalyticsSessionResponse]],
    status_code=status.HTTP_202_ACCEPTED
)
async def process_batch_sessions(
    ...
    face-only tracking, occurrences, and attendance for a list of uploaded CCTV videos.
    """
    tenant_id = verify_tenant(current_user)
    service = FaceAnalyticsService(db)
    ...
    return StandardResponse(
        ...
        status=status.HTTP_202_ACCEPTED,
        data=[FaceAnalyticsSessionResponse.model_validate(s) for s in sessions]
    )
...
@router.get(
    "/sessions/{session_id}",
    response_model=StandardResponse[FaceAnalyticsSessionResponse],
    status_code=status.HTTP_200_OK
)
async def get_session_details(
    ...
    return StandardResponse(
        ...
        status=status.HTTP_200_OK,
        data=FaceAnalyticsSessionResponse.model_validate(session)
    )
